#!/usr/bin/env python
# -*- coding: utf-8 -*-
r"""
检测苹果照片并生成8秒视频
专门针对"C:\Users\李晨鑫\Desktop\苹果照片检测素材"文件夹中的图片
在yolov8目录中运行，避免中文路径问题
"""
import os
import cv2
import numpy as np
from ultralytics import YOLO
import glob
import time

# ============= 可调参数区 =============
# 模型文件路径 (使用当前目录中的模型)
MODEL_PATH = "apple_sensitive.pt"
# 置信度阈值 (0.0-1.0)
CONFIDENCE_THRESHOLD = 0.1
# 目标视频总时长 (秒)
TARGET_VIDEO_DURATION = 8.0
# 输出视频的帧率 (每秒帧数)
VIDEO_FPS = 10
# 检测推理尺寸 (保持与训练一致)
INFERENCE_SIZE = 320
# 输出视频文件名
OUTPUT_VIDEO = "apple_detection_8sec.avi"
# 图片目录 (使用双反斜杠或原始字符串)
IMAGE_DIR = r"C:\Users\李晨鑫\Desktop\苹果照片检测素材"
# 视频编码器 (使用XVID for AVI)
VIDEO_CODEC = 'XVID'
# 统一视频尺寸 (设为None则使用第一张图片的尺寸)
TARGET_SIZE = None  # (宽度, 高度) 例如 (640, 480)
# =====================================

def load_and_sort_images(image_dir):
    """加载并排序图片文件"""
    image_exts = ('*.jpg', '*.jpeg', '*.png', '*.bmp', '*.tiff')
    image_paths = []
    for ext in image_exts:
        image_paths.extend(glob.glob(os.path.join(image_dir, ext)))
    # 按文件名排序
    image_paths.sort()
    print(f"找到 {len(image_paths)} 张图片")
    
    # 显示图片列表
    for i, path in enumerate(image_paths):
        print(f"  {i+1}. {os.path.basename(path)}")
    
    return image_paths

def detect_apples_in_image(model, image_path, conf_threshold):
    """对单张图片进行检测，返回原始图片和检测结果"""
    # 读取图片
    img = cv2.imread(image_path)
    if img is None:
        print(f"无法读取图片: {image_path}")
        return None, None, []
    original_h, original_w = img.shape[:2]
    
    # 转换为RGB用于推理
    img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    
    # 推理
    resized = cv2.resize(img_rgb, (INFERENCE_SIZE, INFERENCE_SIZE))
    results = model(resized, conf=conf_threshold, verbose=False)
    
    boxes = []
    if results and len(results) > 0:
        result = results[0]
        if result.boxes is not None:
            boxes_xyxy = result.boxes.xyxy.cpu().numpy()
            confs = result.boxes.conf.cpu().numpy()
            
            scale_x = original_w / INFERENCE_SIZE
            scale_y = original_h / INFERENCE_SIZE
            
            for i, box in enumerate(boxes_xyxy):
                x1, y1, x2, y2 = box
                conf = confs[i]
                # 缩放回原始尺寸
                x1 = int(x1 * scale_x)
                y1 = int(y1 * scale_y)
                x2 = int(x2 * scale_x)
                y2 = int(y2 * scale_y)
                boxes.append((x1, y1, x2, y2, conf))
    
    return img, (original_w, original_h), boxes

def draw_detection_frame(img, boxes, target_size=None):
    """
    绘制检测框到图片上，并调整到目标尺寸
    返回带检测框的BGR图片
    """
    if target_size is not None:
        target_w, target_h = target_size
        img_h, img_w = img.shape[:2]
        # 调整图片大小
        img_resized = cv2.resize(img, (target_w, target_h))
        # 调整检测框坐标
        scale_x = target_w / img_w
        scale_y = target_h / img_h
        scaled_boxes = []
        for (x1, y1, x2, y2, conf) in boxes:
            x1 = int(x1 * scale_x)
            y1 = int(y1 * scale_y)
            x2 = int(x2 * scale_x)
            y2 = int(y2 * scale_y)
            scaled_boxes.append((x1, y1, x2, y2, conf))
        boxes = scaled_boxes
        frame = img_resized
    else:
        frame = img.copy()
    
    for (x1, y1, x2, y2, conf) in boxes:
        # 绘制边界框
        color = (0, 255, 0)  # 绿色 (BGR)
        thickness = 2
        cv2.rectangle(frame, (x1, y1), (x2, y2), color, thickness)
        
        # 标签：苹果 + 置信度
        label = f"apple {conf:.2f}"
        font = cv2.FONT_HERSHEY_SIMPLEX
        font_scale = 0.5
        label_thickness = 2
        (label_w, label_h), baseline = cv2.getTextSize(label, font, font_scale, label_thickness)
        
        # 标签背景
        label_bg_top = max(y1 - label_h - 5, 0)
        label_bg_bottom = y1
        label_bg_left = x1
        label_bg_right = x1 + label_w
        cv2.rectangle(frame, (label_bg_left, label_bg_top), 
                     (label_bg_right, label_bg_bottom), color, -1)
        # 标签文字
        cv2.putText(frame, label, (x1, y1 - 5), 
                   font, font_scale, (0, 0, 0), label_thickness)
    
    # 添加图片信息文本（检测到的苹果数量）
    info_text = f"Apples: {len(boxes)}"
    font = cv2.FONT_HERSHEY_SIMPLEX
    font_scale = 0.7
    thickness = 2
    text_color = (255, 255, 255)  # 白色
    bg_color = (0, 0, 0, 180)  # 半透明黑色背景
    
    # 文字大小
    (text_w, text_h), baseline = cv2.getTextSize(info_text, font, font_scale, thickness)
    
    # 文字背景
    padding = 8
    bg_x1 = 10
    bg_y1 = frame.shape[0] - text_h - padding - 10
    bg_x2 = bg_x1 + text_w + padding * 2
    bg_y2 = frame.shape[0] - 10
    
    # 绘制半透明背景
    overlay = frame.copy()
    cv2.rectangle(overlay, (bg_x1, bg_y1), (bg_x2, bg_y2), (0, 0, 0), -1)
    frame = cv2.addWeighted(overlay, 0.6, frame, 0.4, 0)
    
    # 绘制文字
    cv2.putText(frame, info_text, (bg_x1 + padding, bg_y2 - padding), 
               font, font_scale, text_color, thickness)
    
    return frame

def create_detection_video(image_paths, model, output_video, target_duration=8.0, fps=10, target_size=None):
    """生成检测视频，控制总时长"""
    if not image_paths:
        print("没有图片可以处理")
        return
    
    num_images = len(image_paths)
    # 计算每张图片占用的帧数（向上取整，确保达到或超过目标时长）
    total_frames_needed = int(target_duration * fps)
    # 向上取整
    frames_per_image = max(1, (total_frames_needed + num_images - 1) // num_images)
    
    print(f"图片数量: {num_images}")
    print(f"目标时长: {target_duration}秒")
    print(f"视频帧率: {fps} FPS")
    print(f"需要总帧数: {total_frames_needed}")
    print(f"每张图片帧数: {frames_per_image}")
    print(f"实际总帧数: {frames_per_image * num_images}")
    print(f"实际时长: {frames_per_image * num_images / fps:.2f}秒")
    
    # 确定视频尺寸
    if target_size is None:
        # 使用第一张图片的尺寸
        first_img, first_size, _ = detect_apples_in_image(model, image_paths[0], CONFIDENCE_THRESHOLD)
        if first_img is None:
            print(f"无法读取第一张图片: {image_paths[0]}")
            return
        target_w, target_h = first_size
    else:
        target_w, target_h = target_size
    
    print(f"视频尺寸: {target_w} x {target_h}")
    
    # 创建视频写入器
    fourcc = cv2.VideoWriter_fourcc(*VIDEO_CODEC)
    video_writer = cv2.VideoWriter(output_video, fourcc, fps, (target_w, target_h))
    
    if not video_writer.isOpened():
        print(f"无法创建视频文件: {output_video}")
        return
    
    processed_count = 0
    total_frames_written = 0
    
    for idx, img_path in enumerate(image_paths):
        print(f"处理图片 {idx+1}/{num_images}: {os.path.basename(img_path)}")
        img, size, boxes = detect_apples_in_image(model, img_path, CONFIDENCE_THRESHOLD)
        if img is None:
            continue
        
        # 绘制检测框并调整尺寸
        frame = draw_detection_frame(img, boxes, (target_w, target_h))
        
        # 写入多帧以实现更长停留时间
        for _ in range(frames_per_image):
            video_writer.write(frame)
            total_frames_written += 1
        
        processed_count += 1
    
    # 释放视频写入器
    video_writer.release()
    print(f"视频已保存: {output_video}")
    print(f"成功处理 {processed_count}/{num_images} 张图片")
    print(f"总写入帧数: {total_frames_written}")
    print(f"最终视频时长: {total_frames_written / fps:.2f}秒")

def main():
    print("=" * 60)
    print("苹果照片检测 - 8秒视频生成")
    print("=" * 60)
    print(f"模型: {MODEL_PATH}")
    print(f"置信度阈值: {CONFIDENCE_THRESHOLD}")
    print(f"目标视频时长: {TARGET_VIDEO_DURATION} 秒")
    print(f"视频帧率: {VIDEO_FPS} FPS")
    print(f"图片目录: {IMAGE_DIR}")
    print(f"输出视频: {OUTPUT_VIDEO}")
    print("=" * 60)
    
    # 检查模型文件
    if not os.path.exists(MODEL_PATH):
        print(f"错误: 模型文件 '{MODEL_PATH}' 不存在")
        print(f"请确保在当前目录(yolov8)中运行")
        return
    
    # 检查图片目录
    if not os.path.exists(IMAGE_DIR):
        print(f"错误: 图片目录 '{IMAGE_DIR}' 不存在")
        return
    
    # 加载模型
    print("加载YOLOv8模型...")
    try:
        model = YOLO(MODEL_PATH)
        print("模型加载完成")
    except Exception as e:
        print(f"模型加载失败: {e}")
        return
    
    # 获取图片列表
    image_paths = load_and_sort_images(IMAGE_DIR)
    if not image_paths:
        print("未找到图片")
        return
    
    if len(image_paths) < 2:
        print(f"警告: 只找到 {len(image_paths)} 张图片，视频可能过短")
        print("您可以提供更多素材以获得更好的效果")
    
    # 生成视频
    start_time = time.time()
    create_detection_video(image_paths, model, OUTPUT_VIDEO, 
                          TARGET_VIDEO_DURATION, VIDEO_FPS, TARGET_SIZE)
    elapsed = time.time() - start_time
    print(f"总处理耗时: {elapsed:.2f} 秒")
    print("完成！")

if __name__ == "__main__":
    main()